...
export type LifiChain = BridgeChain;
export type LifiToken = BridgeToken;
export type LifiQuote = BridgeQuote;
...
export interface SwapQuote {
  ...
  steps: NonNullable<BridgeQuote["steps"]>;
...
export async function getQuote(params: QuoteParams): Promise<SwapQuote> {
  const data = await getLifiQuote(params);
...
function normalizeSwapStatus(status?: string): SwapStatus {
...
export class LiFiAdapter implements IBridgeProvider {
  readonly name = "LI.FI";
  readonly id = "lifi";

  getChains(): Promise<BridgeChain[]> {
    return getLifiChains();
  }

  getTokens(chainId: number): Promise<BridgeToken[]> {
    return getLifiTokens(chainId);
  }
  ...
  async executeSwap(quote: BridgeQuote, signer: Signer): Promise<SwapResult> {
    ...
    if (!tx?.to) {
      throw new Error("LI.FI did not return executable transaction data.");
    }
  ...
  async getStatus(txHash: string, fromChainId: number, toChainId: number): Promise<SwapStatus> {
    const status = await getSwapStatus(txHash, fromChainId, toChainId);
    return normalizeSwapStatus(status);
  }

  supportsRoute(_fromChainId: number, toChainId: number): boolean {
    return toChainId !== 204;
  }
}
