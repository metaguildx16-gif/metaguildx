            ...
            <article className="section-card premium-panel">
              ...
              <div className="section-card-body space-y-2">
                ...
                {boxEarningsProgress && (
                  <div style={{fontSize:"0.75rem",color:"#64748b",marginBottom:6,display:"flex",alignItems:"center",gap:6}}>
                    <span style={{display:"inline-block",width:10,height:10,borderRadius:"50%",border:"2px solid #38bdf8",borderTopColor:"transparent",animation:"spin 0.8s linear infinite"}}/>
                    Scanning… {boxEarningsProgress.chunks}/{boxEarningsProgress.total} chunks
                  </div>
                )}
                {isBoxEarningsSyncing && !boxEarningsProgress && (
                  <div style={{fontSize:"0.75rem",color:"#64748b",marginBottom:6,display:"flex",alignItems:"center",gap:6}}>
                    <span style={{display:"inline-block",width:10,height:10,borderRadius:"50%",border:"2px solid #fbbf24",borderTopColor:"transparent",animation:"spin 0.8s linear infinite"}}/>
                    Loading Box Earnings...
                  </div>
                )}
                <div style={{display:"flex",justifyContent:"flex-end",marginBottom:6}}>
                  <button
                    onClick={onRefreshBoxEarnings}
                    disabled={isBoxEarningsSyncing||!!boxEarningsProgress}
                    style={{fontSize:"0.72rem",padding:"3px 10px",borderRadius:6,border:"1px solid #334155",background:(isBoxEarningsSyncing||!!boxEarningsProgress)?"#1e293b":"#0f172a",color:(isBoxEarningsSyncing||!!boxEarningsProgress)?"#475569":"#94a3b8",cursor:(isBoxEarningsSyncing||!!boxEarningsProgress)?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:4}}
                  >
                    {(isBoxEarningsSyncing||!!boxEarningsProgress)
                      ? <><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",border:"1.5px solid #fbbf24",borderTopColor:"transparent",animation:"spin 0.8s linear infinite"}}/> Scanning</>
                      : "↻ Refresh Boxes"}
                  </button>
                </div>
